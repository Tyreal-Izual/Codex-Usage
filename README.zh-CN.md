...
仪表盘服务器运行期间，后台每四小时读取一次源站的两个公开 JSON 数据源，
...
IQ、费用和耗时沿用源站规则，按软件工程与视觉空间两侧的有效题量加权。
综合成本与 `费用 × (分钟 / 10) ** (log(2.5) / log(1.35))` 成正比，最高成本归一为 100。
横轴使用对数刻度；当最小值与其余值相差较大时，通过 `//` 标记压缩区间。
仅绘制同时具备两侧数据且指标完整的模型档位。这里是社区评测结果，
不是个人账户用量或 OpenAI 官方评分。
